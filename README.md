...
### 운임 예측 + 자동 리포팅 — 시황 판단에 사람이 너무 많은 시간을 쓴다

**문제**: 해운 운임 예측은 뉴스·지표·시장 상황을 종합해야 하는데, 분석가가 수작업으로 읽고 판단하는 데 평균 300시간 이상 소요. 정기 리포트 작성도 매번 반복.

**해결**: ML 모델(SetFit)로 뉴스·시황을 자동 분류하고, LLM 에이전트가 브리핑·예측·시나리오 분석·리포트까지 자동 생성하는 파이프라인 구축. 매일 8,000건 글로벌 공급망 이슈를 자동 수집·분석.

**결과**: 의사결정 소요시간 300시간 → 30분 이내. SCFI 등 주요 운임 지수 95% 신뢰구간 월간 예측. 성균관대 협력 SHAP 기반 모델 해석성 논문 공저. 🟢 운영 중
